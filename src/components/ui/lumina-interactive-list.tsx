  ...
  useEffect(() => {
    ...
    const initApplication = async () => {
        ...
        const fragmentShader = `
            ...
            // Simplified stubs for other effects
        ...
        const initRenderer = async () => {
            ...
            if (slideTextures.length >= 2) {
                ...
                // Animate canvas initial scale (zoom out effect)
                gsap.fromTo(canvas, { scale: 1.2, opacity: 0 }, { scale: 1, opacity: 1, duration: 2, ease: "power2.out" });
        ...
        const onResize = () => { if (renderer) { renderer.setSize(window.innerWidth, window.innerHeight); shaderMaterial.uniforms.uResolution.value.set(window.innerWidth, window.innerHeight); } };
  ...
  return (
    <>
      <main className="slider-wrapper" ref={containerRef}>
        ...
        <div className="side-number side-left" id="slideNumber">01</div>
        <div className="side-number side-right" id="slideTotal">06</div>
      ...
      <style jsx global>{`
        ...
        .slide-title {
            ...
            font-size: clamp(2.5rem, 8vw, 5.5rem); /* Reduced to match reference */
            font-weight: 400;
            line-height: 1.1;
            margin-bottom: 2rem;
            text-transform: none;
        ...
        .side-number {
            ...
            font-family: var(--font-sans); /* Switched to sans for a cleaner look */
            font-size: 0.9rem;
            font-weight: 500;
        ...
        .slides-navigation {
            position: absolute;
            bottom: 8%; /* Adjusted for better visibility */
            left: 10%;
            right: 10%;
            display: grid;
